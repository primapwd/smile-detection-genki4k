...
def load_image(folder, min_num_images):
  ...
  for image in os.listdir(folder):
    ...
    try:
      image_data = (imageio.v2.imread(image_file).astype(float) - 
...
with graph.as_default():
  ...
  fc1w = tf.Variable(tf.random.truncated_normal(
      [big_shape, 4096], dtype=tf.float32, stddev=0.1))
  fc1b = tf.Variable(tf.constant(1.0, shape=[4096], dtype=tf.float32))

#   fc2w = tf.Variable(tf.random.truncated_normal(
#     [4096, 4096], dtype=tf.float32, stddev=0.1))
#   fc2b = tf.Variable(tf.constant(1.0, shape=[4096], dtype=tf.float32))

#   fc3w = tf.Variable(tf.random.truncated_normal(
#       [4096, 1000], dtype=tf.float32, stddev=0.1))
#   fc3b = tf.Variable(tf.constant(1.0, shape=[1000], dtype=tf.float32))
  
  fc4w = tf.Variable(tf.random.truncated_normal(
  ...
  def model(data, keep_prob):
    ...
    drop1 = tf.nn.dropout(fc1, rate=1-keep_prob)
...
#     drop3 = tf.nn.dropout(fc3, keep_prob)
  ...
  # Optimizer.
  learning_rate = tf.compat.v1.train.exponential_decay(1e-5, global_step, 1000, 0.85, staircase=True)
